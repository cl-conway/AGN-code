...
def Eracelous_Candidate_Data_Grab(User):

    #Designed to Grab the data URLs for the periodic candidates identified by
    #Eracelous et al.

    #Define the path to the Eracelous Periodic Candidates
    if User == 'N':
        Eracelous_Candidate_File = 'C:/Users/User/Documents/University/Year 4/Project/Eracleous_Data.txt'
    elif User == 'C':
        Eracelous_Candidate_File = 'C:/Users/Christopher/Documents/UNI/Year 4/Project/AGN-code/Eracelous_Data.txt'

    #Bringing in the Eracelous Candidate input Data
    #Eracelous_Input_Data = pd.read_table(Eracelous_Candidate_File, sep=';', header=0)
    delim = (5,30,12,12,101)
    Eracelous_Input = np.genfromtxt(Eracelous_Candidate_File, dtype=str, autostrip=True, delimiter=delim, usecols=[1,2,3])
    Eracelous_Input_Data = pd.DataFrame(Eracelous_Input)
    Eracelous_Input_Data.columns = ['Name', 'RAJ2000', 'DEJ2000']

    Eracelous_Input_Data.Name.str.strip()
    Eracelous_Input_Data.RAJ2000.str.strip()
    Eracelous_Input_Data.DEJ2000.str.strip()

    Eracelous_Input_Data['RAJ2000'] = Eracelous_Input_Data['RAJ2000'].str.replace('h', ' ')
    Eracelous_Input_Data['RAJ2000'] = Eracelous_Input_Data['RAJ2000'].str.replace('m', ' ')
    Eracelous_Input_Data['RAJ2000'] = Eracelous_Input_Data['RAJ2000'].str.replace('s', ' ')
    Eracelous_Input_Data['DEJ2000'] = Eracelous_Input_Data['DEJ2000'].str.replace('d', ' ')
    Eracelous_Input_Data['DEJ2000'] = Eracelous_Input_Data['DEJ2000'].str.replace('m', ' ')
    Eracelous_Input_Data['DEJ2000'] = Eracelous_Input_Data['DEJ2000'].str.replace('s', ' ')

    #Read in the individual column Data
    Names = Eracelous_Input_Data[['Name']].as_matrix()
    RA_Values = Eracelous_Input_Data[['RAJ2000']].as_matrix()
    Dec_Values = Eracelous_Input_Data[['DEJ2000']].as_matrix()
    ...
    if User == 'N':
        writing_file = open('C:/Users/User/Documents/University/Year 4/Project/Raw_Data_Inputs_Eracelous.txt', 'w')
    if User == 'C':
    ...
    for j in range(0, len(Names)):
        ...
        if check_exists_by_link_text(driver,'download') == True:
            #Grabs the URL link to the data from the webpage
            link = driver.find_element_by_link_text('download')
            URL = link.get_attribute("href")

        elif check_exists_by_link_text(driver,'download') == False:
            URL = 'NaN'
    ...
    #End of Eracelous_Candidate_Data_Grab function

def check_exists_by_link_text(drive, text):
    try:
        drive.find_element_by_link_text(text)
...
def main():
    ...
    if(User == 'C'):
        Data_location = 'C:/Users/Christopher/Documents/UNI/Year 4/Project/AGN-code/Raw_Data_Inputs_Eracelous.txt'
    if(User == 'N'):
        Data_location = 'C:/Users/User/Documents/University/Year 4/Project/Raw_Data_Inputs_Eracelous.txt'

    #Calls the Data Grab function
    #Eracelous_Candidate_Data_Grab(User)

    #Read in the objects and URLs to be searched
    Raw_Data_input = pd.read_table(Data_location, sep=',', header=0)
    Raw_Data_input = Raw_Data_input[Raw_Data_input.URL.str.contains('NaN') == False]
    ...
    Period_data = open('No_of_periods_Eracleous.txt','w')
    ...
    for k in range(0, 10):
        ...
        #Continue past the iterations with NaN URL's
        if URL_editted == 'NaN':
            continue
