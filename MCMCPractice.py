...
# Excersise 1
x_1= np.delete(x, range(4), None)
y_1= np.delete(y, range(4), None)
yerr_1= np.delete(yerr, range(4), None)
xerr_1= np.delete(xerr, range(4), None)

A = np.vstack((np.ones_like(x_1), x_1)).T
C = np.diag(yerr_1 * yerr_1)
cov = np.linalg.inv(np.dot(A.T, np.linalg.solve(C, A)))
b_ls, m_ls = np.dot(cov, np.dot(A.T, np.linalg.solve(C, y_1)))

xlin = np.linspace(0, np.max(x), 1000)
...
def lnprior(theta):
    m, b, P_b, Y_b, lnV_b = theta
    if (0.0 < m <5.0 and -100 < b <100 and 0 < P_b < 1 and 0 <Y_b< 600 and 0 < lnV_b < 10):
        return 0.00
    return -np.inf
...
MCinit = np.array([m_ls, b_ls, 0.1, 400, 5])
ndim, nwalkers = 5, 300
pos = [MCinit+ 1e-4*np.random.randn(ndim) for i in range(nwalkers)]
"""
